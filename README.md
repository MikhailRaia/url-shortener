...
Для запуска сервера с поддержкой HTTPS используйте флаг `-s` или переменную окружения `ENABLE_HTTPS=true`:

```bash
./shortener -s
```

При включении HTTPS сервер автоматически сгенерирует самоподписанные сертификаты (`cert.pem` и `key.pem`), если они отсутствуют в корневой директории.
